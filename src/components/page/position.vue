<template>
  <div id="app">
    <h3 class="title" style="   padding-bottom: 10px">{{ msg }}<button   v-on:click="chosepoint">{{xuandian}}</button></h3>
    ...
    <div class="amap-wrapper" >
      ...
      <el-amap  vid="amap" class="amap-box" :plugin="plugin" :center="center" :zoom="zoom" :events="events" :amap-manager="amapManager">
        ...
        <el-amap-marker  :content="content"  :position="position1" ></el-amap-marker>
...
export default { 
  data () {
    ...
    return {
      ...
      pros2 : ["北京市","天津市","上海市","重庆市","河北省","山西省","辽宁省","吉林省","黑龙江省","江苏省","浙江省","安徽省","福建省","江西省","山东省","河南省","湖北省","湖南省","广东省","海南省","四川省","贵州省","云南省","陕西省","甘肃省","青海省","内蒙古自治区","广西壮族自治区","西藏自治区","宁夏自治区","新疆维吾尔自治区","香港特别行政区","澳门特别行政区","台湾省"],
      ...
      events: {
             click(e){
               if(that.Ischosepoint)
               {
                ...
                //         '</div>';
                that.position1=[lon, lat],
                geocoder.getAddress(lnglat, function(status, result) {
                  if (status === 'complete' && result.info === 'OK') {
                      // result为对应的地理位置详细信息
                      console.log(result);
                      console.log(result.regeocode.addressComponent.province);
                      that.showInfoClick(result.regeocode.addressComponent.province,lon,lat);
                    }
                  })
                })
  ...
  methods: {
    chosepoint(){
      if(this.Ischosepoint)
      {
        console.log("5");
        this.Ischosepoint=false;
        this.xuandian="选点";
        this.position1=[0,90];
        this.showDetails(this.tableData);
      }
      else
      {
        console.log("6");
        this.Ischosepoint=true;
        this.xuandian="取消";

      }
    },
            //地图点击执行事件
          showInfoClick(province,lon,lat){
            //that.markers=[];
            //that.tableData=[];
            //that.points=[];
            var that=this;
            ...
            //             '</div>',
            // }
            // that.areas.push(mark);
            for(var i=0;i<34;i++)
            {
              //console.log(province.indexOf(that.pros2[j]));
              if(province==that.pros2[i])
              {
                console.log("北京");
                flag=true;
                index=i;
            ...
            {
              var area=0;
              var data=that.firstdata[index]
              for(var j=0;j<data.length;j++)
              {
                
                area=(data[j].Longitude-lon)*(data[j].Longitude-lon)+(data[j].Latitude-lat)*(data[j].Latitude-lat)

                if(area<1)
                {
                  that.areas.push(data[j]);
                }
              }
              that.showDetails(that.areas);
            }
          },
           getData() {
            console.log("56");
                ...
                if(length>0)
                {
                    ...
                    axios.post(localStorage.getItem('urls')+"api/Basestation/Mountpoints",
                                qs.stringify({
                                    ...
                                    that.showDetails(that.tableData);
                                    }).catch(() => {});
                }
            })
        ...
        searchinf:function(){
          ...
          {
            if(that.tableData[i].Mountpoint==this.searchtext)
            {
              flag=true;
              break;
            }
        ...
        showDetails:function(tableData){
